...
- **Buffer replay (session restore / reopen tab) is escape-sequence-sensitive.** The replay in `Terminal.tsx` appends `\r\n` (so the fresh shell doesn't print zsh's `%` partial-line marker) and DECSC `\e7` (the shell's own save-cursor is emitted before the data listener attaches and is lost; an unpaired DECRC `\e8` would otherwise home to (0,0) and the following `\e[J` wipes the restored screen — seen with powerlevel10k). Don't remove either without re-testing reopen with p10k.
- **Never let an exception escape an `xterm.write` callback** — it corrupts xterm's write-queue processing. `scrollToBottom()` throws while a fresh mount's renderer has no dimensions; use the local `safeScrollToBottom` pattern.
- **In `closeTab`-style flows, remove state before killing PTYs.** Kills emit `pty://…/exit` events; `handleTerminalExit` reacting to a half-closed tab re-enters close logic (this once produced duplicate, degraded entries on the reopen stack).
- **The macOS "click into a background window's panel needs two clicks" symptom is `acceptFirstMouse`, not a JS focus race.** By default macOS swallows the window-activating click instead of delivering it to the webview, so `focusPanel` never fires and focus stays on the old panel. Fixed by `"acceptFirstMouse": true` in the window config (`src-tauri/tauri.conf.json`). Don't try to fix this in the focus/rAF code — the click never reaches the DOM. (Diagnostic: does the `.panel--focused` ring move on the first click? If not, it's the click being dropped, not focus timing.)
- **`acceptFirstMouse` has a flip side: the activating click now reaches xterm and is forwarded to a mouse-tracking TUI** (interactive prompt, `fzf`, `vim -c 'set mouse=a'`), so switching back from another window can accidentally pick an option. `Terminal.tsx` suppresses the mouse report for that one click via a capture-phase `mousedown` listener + `stopImmediatePropagation` (still calls `xterm.focus()` so typing lands), gated on `isActivatingClick()` from `src/utils/windowActivation.ts` (tracks window `focus`/`blur` + a 250 ms grace). Only the left button, only the activating click — every later click passes through. These two bullets are a pair: don't remove the suppression while keeping `acceptFirstMouse`.
